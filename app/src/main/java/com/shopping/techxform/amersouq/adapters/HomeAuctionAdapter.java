...
import com.shopping.techxform.amersouq.RetrofitHelpers.Models.HomePageData.AuctionsItem;
...
public class HomeAuctionAdapter extends RecyclerView.Adapter<HomeAuctionAdapter.ViewHolder> {
    ...
    private List<AuctionsItem> namelist = new ArrayList<>();

    Context context;

    Activity activity;

    public HomeAuctionAdapter(List<AuctionsItem> namelist, Activity activity, Context context) {
    ...
    public List<AuctionsItem> getStudentist() {
        return namelist;
    }
...
//                            Toast.makeText(getActivity(),responseBody.getClassified().getImages().get(0),Toast.LENGTH_SHORT).show();
        ...
        holder.full_layout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                intent.putExtra("ad_id",namelist.get(position).getAuctionAdId());
